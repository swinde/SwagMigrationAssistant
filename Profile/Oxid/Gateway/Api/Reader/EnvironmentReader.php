...
use Oxid\Core\Framework\OxidHttpException;
...
use SwagMigrationAssistant\Profile\Oxid\Exception\PluginNotInstalledException;
...
class EnvironmentReader implements EnvironmentReaderInterface
{
    ...
    public function read(MigrationContextInterface $migrationContext): array
    {
        ...
        if ($this->doOxidCheck($information)) {
            return $information;
        }
    ...
    private function doSecureCheck(array &$information): bool
    {
        ...
        try {
            ...
            return true;
        } catch (OxidHttpException $eVerified) {
    ...
    private function doInsecureCheck(array &$information): bool
    {
        ...
        try {
            ...
            return true;
        } catch (OxidHttpException $eUnverified) {
            return false;
        }
    }

    private function doOxidCheck(array &$information): bool
    {
        if ($this->client === null) {
            return false;
        }
        ...
        try {
            if ($this->checkForOxid($this->client)) {
                throw new PluginNotInstalledException();
            }

            throw new GatewayReadException('Oxid 5.5 Api SwagMigrationEnvironment', 466);
        } catch (OxidHttpException $eOther) {
    ...
    private function checkForOxid(Client $apiClient): bool
    {
